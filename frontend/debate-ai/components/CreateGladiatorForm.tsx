    ...
    startTransition(async () => {
      try {
        ...
        const gladiatorData: GeneratedGladiator = {
          ...
          description:
            characterData.data.bio?.join(" ") || "No description available",
          speciality: characterData.data.topics?.[0] || "General Philosophy",
          stats: {
            strength: Math.min(
              100,
              (characterData.data.topics?.length || 0) * 20
            ),
            agility: Math.min(
              100,
              (characterData.data.postExamples?.length || 0) * 5
            ),
            intelligence: Math.min(
              100,
              (characterData.data.adjectives?.length || 0) * 25
            ),
          },
